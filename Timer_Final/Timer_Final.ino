...
void loop() {
  ...
  if (currentMillis - previousMillis > interval - 3.33333) {
    ...
    if (mode == timermode) {
      ones++;
      if (secondsbeep >= seconds - 4 && hundred == z && secondsbeep < seconds - 1 && xx > 0) {
        toney(100);
      }
    }
  }
